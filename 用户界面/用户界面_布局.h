...
public:
	...
	S布局参数 f生成矩形(float x倍数 = 0, float y倍数 = 0) const;
	S布局参数 f生成正方形(float x倍数 = 0, float y倍数 = 0) const;
	S布局参数 f移动生成矩形();
	S布局参数 f移动生成正方形();
	t向量2 m坐标{};
	t向量2 m大小{};
...
class C表格排行布局 {
public:
	void f属性_s布局(const t向量2 &坐标, const t向量2 &父尺寸, float 子高度);
	S布局参数 f生成(float y偏移 = 0);
	t向量2 m坐标{};
	t向量2 m父尺寸{};
	float m子高度 = 0;
	float m上 = 0;
...
class C表格排列布局 {
public:
	void f属性_s布局(const t向量2 &坐标, const t向量2 &父尺寸);
	S布局参数 f生成(float x移动 = 0);
	t向量2 m坐标{};
	t向量2 m父尺寸{};
	float m左 = 0;
};
