...
void C用户界面::f计算() {
	...
	const auto f更新鼠标焦点 = [this](const t向量2 &a坐标) {
		...
		for (auto &vp窗口 : ma窗口表) {
			...
			const bool vi范围内 = vp窗口->f响应_i范围内(v相对坐标);
			vp窗口->m标志[W窗口::e鼠标范围] = vi范围内;
			if (vi范围内) {	//如果有多个窗口在范围内,选择最深的窗口
				if (v目标窗口) {
					if (v目标窗口->m窗口层 < vp窗口->m窗口层) {
						v目标窗口 = vp窗口;
					}
				} else {
					v目标窗口 = vp窗口;
...
void C用户界面::f新建窗口(W窗口 &a窗口) {
	...
	if (auto v找 = ma活动窗口.find(&a窗口); v找 != ma活动窗口.end()) {
		...
		if (!m按键焦点) {	//没有按键焦点,拿当前窗口作为焦点(可能有问题)
			m按键焦点 = m鼠标焦点 = &a窗口;
			a窗口.m标志[W窗口::e焦点] = true;
			a窗口.f响应_焦点变化();
		}
	}
}
