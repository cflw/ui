...
void W空白按钮::f动作_按键(E按键 a按键) {
	f动作_按键(S按键参数{E按键来源::e无, a按键, t向量2::c零});
}
//==============================================================================
// 标签
//==============================================================================
...
void W标签::f响应_显示(const S显示参数 &a) const {
	a.m画界面.f绘制文本(m文本, fg动画矩形(), a.m主题.fg颜色(1, 1, fg总切换().fg透明度() * m透明度), {m字号, m对齐});
}
...
void W垂直滚动条::f属性_s布局(W窗口 &a窗口, float a上偏移, float a下偏移) {
	t向量2 m坐标0{(a窗口.m尺寸.x - c边长) * 0.5f, a上偏移 + a下偏移};
...
int W垂直滚动条::f属性_g滑块位置() const {
	...
	return 数学::f限制<int>(v滑块位置, 0, v最大值);
}
