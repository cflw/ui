...
void W选项卡组::f动作_切换(int a序号) {
	...
	if (m页序号 != -1) {
		S页 &v当前页 = ma页[m页序号];
		v当前页.m窗口->f动作_隐藏();
		v当前页.m按钮->m标志[e活动] = false;
	}
	S页 &v切换页 = ma页[a序号];
	v切换页.m窗口->f动作_显示();
	v切换页.m按钮->m标志[e活动] = true;
	m页序号 = a序号;
}
...
S布局参数 W选项卡组::f属性_g页按钮布局(int a序号) const {
	const int v上 = m尺寸.y * 0.5f;
	const int v下 = v上 - m页按钮尺寸.y;
	const int v左 = -m尺寸.x * 0.5f + m页按钮尺寸.x * a序号;
	const int v右 = v左 + m页按钮尺寸.x;
	S布局参数 v;
	v.m尺寸 = t向量2::fc矩形尺寸(v左, v上, v右, v下);
	v.m坐标 = t向量2::fc矩形中心(v左, v上, v右, v下);
	return v;
}
void W选项卡组::f重置页按钮布局() {
	const int v上 = m尺寸.y * 0.5f;
	...
	for (const auto &[v窗口, v按钮] : ma页) {
		...
		const int v右 = v左 + m页按钮尺寸.x;
		v按钮->f属性_s布局(t向量2::fc矩形中心(v左, v上, v右, v下), t向量2::fc矩形尺寸(v左, v上, v右, v下));
		++i;
	}
}
}	//namespace 用户界面