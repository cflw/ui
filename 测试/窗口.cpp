...
W窗口2::W窗口2() {
	用户界面::C单向移动布局 v布局;
	v布局.f属性_s布局(t向量2(-100, 100), t向量2(20, 20));
	v布局.f属性_s倍数移动(1, 0);
	for (int i = 0; i != 10; ++i) {
...
W窗口4::W窗口4() {
	constexpr int c数量 = 4;
	for (int i = 0; i != c数量; ++i) {
		用户界面::W标签 &v标签 = ma标签.emplace_back(0, i);
		v标签.f属性_s文本内容(std::to_wstring(i));
		v标签.f属性_s布局(t向量2::c零, t向量2(100, 100));
	}
	w选项卡组.f属性_s布局(t向量2::c零, t向量2(200, 200));
}
void W窗口4::f响应_初始化() {
	for (用户界面::W标签 &v标签 : ma标签) {
		w选项卡组.f动作_添加页(v标签, v标签.m文本);
	}
	f动作_添加窗口(w选项卡组);
	w选项卡组.f动作_显示();
	f动作_显示();
